...
\subsection{Hematopoietic stem cell cell differentiation}
...
Immunoglobulins (Ig) are typically large Y-shaped proteins, consisting of two identical heavy chains and two identical light chains (Figure \ref{fig:immunoglobulins_diagram}a).
%%
%% Antibody figure
\begin{figure}[htb]
...
Humans produce five isotypes of immunoglobulins: IgM, IgD, IgG, IgA and IgE (named after their respective heavy chain type; Figure \ref{fig:immunoglobulins_diagram}b).
Igs can either be membrane-bound (mIgs) on the cell-surface or secreted (sIgs, also called antibodies).
Newly formed B cells produce IgMs only.
IgM molecules are inserted into their plasma membrane, where they act as B-cell receptors (BCRs) \cite{friess2018structural}.
...
Moreover, diversity of antigen-binding site is further increased by junctional diversification, whereby variable numbers of nucleotides are lost or randomly inserted at joining sites.
This can increase diversity of variable coding sequences by $10^8$ fold.
However, this process can also shift the reading frame to produce non-functional genes, meaning the given B cell cannot produce functioning Ig and subsequently die in the bone marrow.
Once a B cell can produce functional Igs, the V(D)J recombination process is switched off, and it only makes Igs with the same antigen-binding site.

\subsubsection{B cell maturation}
As B cells develop and leave the bone marrow to populate peripheral lymphoid tissues, they start to express IgD in addition to IgM and form IgD BCRs, they are then known as mature naive B cells.
The IgM and IgD BCRs on these cells will contain the same antigen-binding site, but with different constant heavy chains.
B cells require antigen stimulation to fully differentiate.
Upon antigen stimulation, mature naive B cells are activated to become Ig-secreting effector cells (plasma cells) or memory B cells.
...
Class switching is dependent on the mechanism switch recombination, where the constant heavy chain region undergoes a number of DNA cutting/rejoining events that bring any of the secondary Igs constant heavy chain exons adjacent to heavy VDJ exon.
The mechanism is not yet fully understood, however AID is necessary to facilitate class switching.
Class switching often occurs after somatic hypermutation has taken place, however they are independent processes, and class switching can take place without having already undergone somatic hypermutation.
Although isotype switching is independent of somatic hypermutation and can occur without it, most B cells that express Igs of new isotypes have already undergone somatic hypermutation.

%Initially, effector B cells will only secrete primary Igs (IgM and IgD), until antigenic stimulation and helper T cell activation, stimulate B cells to undergo class switching and they are then able to produce the secondary class of Igs: IgD, IgA and IgE\@.
These effector B cells will then be able to produce all classes of Igs, all retaining their original specific antigen-binding site.
...
By each Ig class retaining the same antigen-binding site, this means that a specific antigen-binding site for a pathogen can be distributed amongst the various isotypes and can therefore acquire each classes biological properties.

Affinity maturation occurs through a process known as somatic hypermutation, whereby activated B cells form germinal centers and their variable regions mutate at a rate about a million times faster than spontaneous mutations.
...
Ig recognise and bind to the specific foreign antigen on the pathogen which stimulated their production.
