...
Preferential selectivity for MM cells should be confirmed using dose response experiments, using MM cells and MM patient-derived immune cells, separated into their constituent cell types. % FACS?
It would be interesting to see if the effect of ProRS inhibition on myeloid cells extends to healthy individuals or other pathologies.
...
Other tRNA synthetase inhibitors tested in MM\@.

Prolyl-tRNA synthetase (ProRS) is an enzyme which catalyzes the coupling of tRNA\textsuperscript{pro}s to proline, therefore the effect of ProRS inhibitors on tRNAs was of great interest.
A tRNA-seq analysis pipeline (tRNAnalysis) was developed to analyse tRNA-seq data (Section \ref{sec:trna_method}).
