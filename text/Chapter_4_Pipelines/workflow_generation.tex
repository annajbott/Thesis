...
This further affirms the benchmark conducted in this work, its results and the subsequent decision to use Kallisto BUStools as the lightweight mapper of choice for scRNA-seq analysis.

The benchmark performed in this work used F1, precision and recall scores to assess accuracy.
These scores use binary classifications, therefore the benchmark only looked at the accuracy of `Expressed' vs `Not expressed' for every gene for each cell. This was considered an appropriate measure of accuracy, as the fraction of zeros (cells not expressing a given gene) in a scRNA-seq experiment is extremely high, sometimes exceeding 90\%\cite{linderman2022zero}.
...
Srivastava et al. (2020) investigated the transcript abundance quantification performance of traditional mapping and alignment strategies Bowtie2 and STAR compared to the quasi-mapping (pseudoalignment) method Salmon\cite{srivastava2020alignment}. T
he group calculated Spearman correlation of quantification estimates by each alignment method compared to Polyester simulated scRNA-seq ground-truth data.
...
Real world data should also be included in to assess how methods perform with added complexity and heterogeneity, which is not present in simulated data. Since no ground-truth data is available, it becomes more challenging to analyse accuracy.
The strategy implemented in \cite{booeshaghi2021benchmarking} could be generalised to include all traditional mapping and lightweight mapping techniques. Such that a real-world dataset is aligned and processed by the numerous methods, then combined into one counts dataset.
Following dimensionality reduction of this matrix, you would expect methods to cluster together for each given ` doppelganger' cell.
L1 distances from the centroid of these clusters could then be calculated for each method, for each cell in the dataset, to give an overall score for each method and to see how much each method agrees with one another.
This strategy could be implemented for real-world MM data, to ensure the methods are appropriate to analyse MM patient data.
