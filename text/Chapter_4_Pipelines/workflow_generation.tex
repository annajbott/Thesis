...
Many scRNA-seq studies use FCM immunophenotyping and bead enrichment to sort cells prior to sequencing.
...
Currently no MM classifiers exist to automate MM scRNA-seq cell identification.
...
From assessment of the classifiers on test MM experimental scRNA-seq data, there was high agreement between classifier MM identification and gene expression-based cell type identification.
However, the ground-truth data was defined by me based on cell type classifications using gene expression data.
As the classifier was trained using gene expression on other MM scRNA-seq, also defined by me...
SELF BIAS- assign the same things... <HERE WORD BETTER ME???>
The performance of the classifier is dependent on the data used to train the model.
The classifier generated in this work is based on three experiments, consisting of BM samples from four patients, two of whom being newly-diagnosed MM patients, and two being relapsed patients, totalling approximately 80,000 cells (Table \ref{tab:cells_passing}).
...
The group investigated the impact of CD138\textsuperscript{+} magnetic bead-based selection compared to whole bone marrow (WBM) processing on BM plasma cell surface markers using FCM\cite{bansal2021impact}.
...
For DNA and RNA-based studies (like scRNA-seq) it is very concerning that CD138\textsuperscript{+} selection could preferentially select for a subset of cells, as this would mean further heterogeneity is being lost and results are being skewed to a subset of total MM cells.
...
Sequencing the WBM niche is more expensive than sequencing isolated PCs (if maintaining a good read depth and high cell count);
...
The MM classifiers created in this work are hosted publically on Github (), along with code to generate new classifiers, so that other researchers can access and use them if they wish.
...
ML classifiers based on scRNA-seq datasets would likely have more power and stronger predictive capabilities than classifiers built using bulk RNA-seq data, due to the underlying heterogeneity of the patient data.
