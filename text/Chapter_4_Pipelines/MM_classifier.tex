...
Therefore, it can be concluded that clusters 4, 7, 9 and 10 are the MM cell population.
...
To quantify each model's accuracy of classifying MM cells, precision, recall and F1 scores were calculated for each of the three models (Figure \ref{fig:mm_class_accuracy_bar}).
%
\begin{figure}[htb]
\centering
...
This should be bore in mind when using the MM classifiers in future, applying the correct classifier to a dataset, such that the naive model is applied to newly-diagnosed MM patient datasets and the relapsed model to RRMM patient datasets.
