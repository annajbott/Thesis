...
PI-sensitive cells are referred to as WT cells in this work.
...
This was due to the drug exposure selecting the L363 contaminate population over the AMO-1 cells, due to their innate increased resistance to PI, compared to AMO-1 cells.
...
Stock concentrations of PIs, bortezomib (BTZ) and carfilzomib (CFZ) were purchased from Thermofisher, UK.
...
DMSO concentration was kept the same ($\leq0.1$\%) for every sample across an experiment, to minimise its toxic effects and to keep its effects consistent across conditions.
...
Fasta files were processed using CGAT-flow\cite{sims2014cgat}, the workflow can be found at: \url{https://github.com/cgat-developers/cgat-flow/blob/master/cgatpipelines/tools/pipeline_rnaseqdiffexpression.py}.
...
Downstream analysis was performed in Jupyter lab notebooks\cite{Kluyver2016jupyter} with R kernels.
